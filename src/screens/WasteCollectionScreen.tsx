...
import {clearAutoGeneratedDocumentPhotos} from '../services/autoGeneratedDocumentPhotos';
...
import {ServiceTypeCompleteValidationModal} from './waste-collection/ServiceTypeCompleteValidationModal';
...
import type {NoShipFormState} from './waste-collection/NoShipDocumentScreen';
import {createEmptyNoShipFormState} from './waste-collection/NoShipDocumentScreen';
...
const SYNC_SUPPORT_PHONE_DISPLAY = '1-800-555-1212';
const SYNC_SUPPORT_PHONE_TEL = 'tel:18005551212';
...
const WasteCollectionScreen: React.FC<WasteCollectionScreenProps> = ({
  ...
  const [showSyncSupportPhone, setShowSyncSupportPhone] = useState(false);
  ...
  const [sideQuestReturnStep, setSideQuestReturnStep] =
    useState<FlowStep>('stream-selection');
  const [photosInitialCategory, setPhotosInitialCategory] =
  ...
  const [showServiceTypeCompleteValidationModal, setShowServiceTypeCompleteValidationModal] =
    useState(false);
  ...
  const [noShipFormByOrder, setNoShipFormByOrder] = useState<
    Record<string, NoShipFormState>
  >({});
  ...
  const currentNoShipFormState = useMemo(() => {
    const orderNumber = selectedOrderData?.orderNumber;
    if (!orderNumber) {
      return createEmptyNoShipFormState(username);
    }
    return (
      noShipFormByOrder[orderNumber] ?? createEmptyNoShipFormState(username)
    );
  }, [selectedOrderData?.orderNumber, noShipFormByOrder, username]);

  const handleNoShipFormStateChange = useCallback(
    (next: NoShipFormState) => {
      const orderNumber = selectedOrderData?.orderNumber;
      if (!orderNumber) {
        return;
      }
      setNoShipFormByOrder(prev => ({
        ...prev,
        [orderNumber]: next,
      }));
    },
  ...
  useEffect(() => {
    if (!showPostLoginSyncOverlay) {
      setShowSyncSupportPhone(false);
    }
  ...
  /**
   * Soft-validation gate before marking a service type complete.
   * Opens the combined Materials/Equipment/Photos reminder; completion runs only on Yes.
   */
  const serviceTypeCompleteValidationConfirmRef = useRef<(() => void) | null>(
    null,
  );

  const handleServiceTypeCompleteValidationNo = useCallback(() => {
    serviceTypeCompleteValidationConfirmRef.current = null;
    setShowServiceTypeCompleteValidationModal(false);
  }, []);

  const handleServiceTypeCompleteValidationYes = useCallback(() => {
    const confirm = serviceTypeCompleteValidationConfirmRef.current;
    serviceTypeCompleteValidationConfirmRef.current = null;
    setShowServiceTypeCompleteValidationModal(false);
    confirm?.();
  }, []);

  /** Mark the current service type complete (end timer); navigate to Order Information (dashboard). */
  const confirmMarkServiceTypeComplete = useCallback(async () => {
  ...
  const handleMarkServiceTypeComplete = useCallback(() => {
    if (!selectedOrderData || !activeServiceTypeTimer) return;
    serviceTypeCompleteValidationConfirmRef.current = () => {
      void confirmMarkServiceTypeComplete();
    };
  ...
  const confirmVoidManifest = useCallback(() => {
    const orderNumber = manifestOrderNumber;
    setManifestTrackingNumber(null);
    setManifestOrderNumber(null);
    setManifestData(null);
    setSelectedPrograms({});
    ...
    if (orderNumber) {
      void clearAutoGeneratedDocumentPhotos(orderNumber);
    }
  }, [manifestOrderNumber]);
    ...
    return (
      <View style={styles.quickActionsBar}>
        ...
        <TouchableOpacity
          ...
          onPress={() => !materialsDisabled && navigateToSideQuest('materials-supplies')}
          activeOpacity={0.7}
        ...
        <TouchableOpacity
          ...
          onPress={() => !equipmentDisabled && navigateToSideQuest('equipment-ppe')}
          activeOpacity={0.7}
      ...
      case 'noship-preview':
        return (
          <ExtNoShipDocumentScreen
            ...
            technicianName={username}
            hasManifestForOrder={hasManifestForOrder}
            formState={currentNoShipFormState}
            onFormStateChange={handleNoShipFormStateChange}
          />
        );
      ...
      case 'order-service':
        return (
          <ExtOrderServiceScreen
            ...
            onBack={() =>
              setCurrentStep(
                currentOrderNoShipItems.length > 0
                  ? 'noship-preview'
                  : 'manifest-management',
              )
            }
  ...
  const handleContactSupport = useCallback(() => {
    setShowSyncSupportPhone(true);
  }, []);

  const handleCallSupport = useCallback(() => {
    Linking.openURL(SYNC_SUPPORT_PHONE_TEL).catch(() => {});
  }, []);

  const renderPostLoginSyncSupportRow = () => (
    <View style={styles.postLoginSyncSupportRow}>
      {showSyncSupportPhone ? (
        <>
          <Text style={styles.postLoginSyncSupportLabel}>Contact support at </Text>
          <TouchableOpacity
            onPress={handleCallSupport}
            activeOpacity={0.7}
            accessibilityRole="link"
            accessibilityLabel={`Call ${SYNC_SUPPORT_PHONE_DISPLAY}`}>
            <Text style={styles.postLoginSyncSupportLink}>
              {SYNC_SUPPORT_PHONE_DISPLAY}
            </Text>
          </TouchableOpacity>
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <ServiceTypeCompleteValidationModal
        visible={showServiceTypeCompleteValidationModal}
        onNo={handleServiceTypeCompleteValidationNo}
        onYes={handleServiceTypeCompleteValidationYes}
      />
