...
import {ensureLdrDocumentPhoto} from '../../services/autoGeneratedDocumentPhotos';
...
export const OrderServiceScreen: React.FC<OrderServiceScreenProps> = ({
  ...
  // Auto-generate LDR once manifest exists and customer acknowledgement is complete.
  useEffect(() => {
    const orderNumber = selectedOrderData?.orderNumber;
    if (!orderNumber || !manifestTrackingNumber) {
      return;
    }
    if (!hasManifestForOrder(orderNumber)) {
      return;
    }
    const first = customerFirstName.trim();
    const last = customerLastName.trim();
    if (!first || !last || !customerSignature) {
      return;
    }

    const wasteCodes = Array.from(
      new Set(
        addedContainers.flatMap(container => container.wasteCodes ?? []),
      ),
    );

    void ensureLdrDocumentPhoto({
      orderNumber,
      trackingNumber: manifestTrackingNumber,
      customerName: `${first} ${last}`,
      technicianName: technicianName?.trim(),
      signatureDataUri: customerSignature,
      wasteCodes,
    });
  }, [
    selectedOrderData?.orderNumber,
    ...
    if (selectedOrderData?.orderNumber) {
      photoService
        ...
        .forEach(category => {
          ...
          const isAutoGenerated = isAutoGeneratedPhotoCategory(category);
          ...
          reasons.push({
            ...
            reason: isAutoGenerated
              ? `${label} document will be generated after customer name and signature are captured`
              : isSlotted
            ...
            hint: isAutoGenerated
              ? 'Tap to go to customer name and signature'
              : isSlotted
