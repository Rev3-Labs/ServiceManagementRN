...
export const PHOTO_CATEGORY_DEFINITIONS: ReadonlyArray<PhotoCategoryDefinition> =
  [
    ...
    },
    {
      ...
      autoGenerated: true,
      description:
        'Land disposal restriction form (auto-generated when manifest is created)',
    },
    {
      category: 'no-ship',
      label: 'No Ship',
...
class PhotoService {
  ...
  getMissingShippingDocumentCategories(orderNumber: string): PhotoCategory[] {
    return this.getShippingDocumentCategories().filter(category => {
      // Auto-generated docs (LDR / No Ship) are created by the app, not manually.
  ...
  getMissingCloseoutPhotoCategories(orderNumber: string): PhotoCategory[] {
    return this.getCloseoutRequiredCategories().filter(category => {
      // Never surface auto-generated documents (e.g. LDR) as missing closeout items.
      if (isAutoGeneratedPhotoCategory(category)) {
        return false;
      }
      return !this.hasCategoryPhoto(orderNumber, category);
    });
  }
